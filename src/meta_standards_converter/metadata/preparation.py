...
def library_evidence_levels(sample, channel=None, run=None, *, series=None):
    """Return ordered evidence scopes shared by biological and rendering decisions.

    Scope precedence: library/channel identity, sample identity, bound preparation,
    structured source. Study-wide prose and renderer fallback are deliberately absent.
    """
    run = run or {}
    prefix = f"sample[{sample.get('iid') or 'unknown'}]"
    run_prefix = prefix + f".run[{run.get('run') or run.get('experiment') or 'unknown'}]"
    levels = [[], [], [], []]
    def add(level, path, value):
        if value is not None and str(value).strip():
            levels[level].append((path, str(value)))
    for key in ('library_name', 'description'):
        add(0, run_prefix+'.'+key, run.get(key))
    channels = [channel] if channel is not None else sample.get('channel', [])
    for i, item in enumerate(channels):
        index = next((j for j, c in enumerate(sample.get('channel', [])) if c is item), i)
        path = prefix + f'.channel[{index}]'
        for j, characteristic in enumerate(item.get('characteristics', [])):
            tag = re.sub(r'[\s_-]+', '_', str(characteristic.get('name') or characteristic.get('tag', '')).strip().casefold())
            if tag in {'assay', 'assay_type', 'library_type', 'library_name', 'technology',
                       'control', 'control_type', 'singlecell_type', 'library_chemistry', 'chemistry'}:
                add(0, path+f'.characteristics[{j}].value', characteristic.get('value'))
        add(2, path+'.extract_protocol', item.get('extract_protocol'))
    for key in ('title', 'description', 'library_name', 'iid'):
        add(1, prefix+'.'+key, sample.get(key))
    for key in ('library_construction_protocol', 'library_protocol'):
        add(2, run_prefix+'.'+key, run.get(key))
    for path, value in bound_protocols(sample, run, series):
        add(2, path, value)
    add(3, run_prefix+'.library_source' if run.get('library_source') else prefix+'.library_source',
        run.get('library_source') or sample.get('library_source'))
    role = control_role(sample, channel, run)
    levels[2] = [(path, control_preparation(value, role)) for path, value in levels[2]]
    return levels
