...
The converter-owned observation namespace emits no `geo_*` aliases. Every established `msc_*` column has an additive dotted alias grouped under `msc.sample`, `msc.series`, `msc.platform`, `msc.archive`, `msc.library`, `msc.instrument`, `msc.protocol`, `msc.database`, `msc.asset`, or `msc.expression`; both forms carry identical values during this compatibility stage. Stable columns cover sample/study accessions, title/description, organism and taxid, organism part, developmental stage, disease, genotype, biological source, material/provider/molecule, platform, SRA/ENA/BioSample/run accessions, library fields, instrument, modality, asset provenance, and the MINiML database identity. `msc_organism`/`msc.sample.channel.organism.value` resolves each channel independently, preferring non-empty `hz_organism` container-list or scalar values before falling back to the original `organism`; missing values stay empty and repeated labels are de-duplicated case-insensitively in source order. The original organism taxid continues to populate its separate taxid fields. `msc_metadata_source`/`msc.database.identifier` is derived from `database.public_id`, then `iid`, then `name`; its name and URI accompany it. Every characteristic becomes both `msc_characteristic_<normalized_tag>` and `msc.characteristics.<normalized_tag>`, including `hz_*`, `hz_*_id`, and `hz_*_onto` tags emitted by ontology harmonization. Values from repeated tags or channels are de-duplicated case-insensitively and joined in source order, while the union of study tags is present on every sample. Existing source-H5AD observation columns remain intact. Observation names already equal to or suffixed by their GSM accession are preserved; other names receive a `-GSM...` suffix. Combined H5ADs use both `msc_batch` and `msc.combination.batch`. Study-level H5AD splitting prefers `msc.sample.accession` and retains all previous accession-column fallbacks.
